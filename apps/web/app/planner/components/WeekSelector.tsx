...
export function WeekSelector({
  ...
  useEffect(() => {
    ...
    const activeEl = viewport.querySelector<HTMLElement>(
      `[data-week='${activeWeekStart}']`,
    );
    if (!activeEl) return;
    const offset =
      activeEl.offsetLeft + activeEl.offsetWidth / 2 - viewport.clientWidth / 2;
  ...
  return (
    <div className="relative flex items-center">
      ...
      <ScrollArea
        ref={viewportRef}
        className="w-full mx-0 sm:mx-14"
      ...
      >
        <div className="flex items-center gap-3 px-4 py-1 min-h-[44px]">
          {weeks.map((entry) => {
            ...
            const distance =
              entry.index !== null && activeWeekIndex !== -1
                ? Math.abs(entry.index - activeWeekIndex)
                : null;
