...
import { everydayLabel, CategoryEmoji } from "./labels";
...
export function RecipesEverydayBoard() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (!recipe || recipe.everydayScore === targetScore) return;
      patchField.mutate({ id: recipeId, everydayScore: targetScore });
    },
    [recipes, patchField],
  );
  ...
  return (
    <div className="flex flex-col gap-3">
      <ColumnFilter
        ...
        getLabel={(s) => everydayLabel(Number(s))}
      />
      <DndContext
        sensors={sensors}
