...
export function IngredientsDuplicateBoard() {
  const utils = trpc.useUtils();
  const { data, isLoading } = trpc.ingredient.listPotentialDuplicates.useQuery();
  const mergeMutation = trpc.ingredient.merge.useMutation({
    onSuccess: () => {
      utils.ingredient.listPotentialDuplicates.invalidate();
  ...
  const handleMerge = useCallback(
    (groupIndex: number, group: DuplicateGroup) => {
      ...
      const mergeIds = group.filter((item) => item.id !== keepId).map((item) => item.id);
  ...
  if (isLoading) {
    return <p className="p-4 text-muted-foreground">Laster duplikatgrupper…</p>;
  }
  ...
  if (groups.length === 0) {
    return (
      <div className="rounded-xl border border-border/50 bg-card/60 p-8 text-center">
        <p className="text-lg font-medium text-card-foreground">✅ Ingen duplikater funnet</p>
        <p className="mt-1 text-sm text-muted-foreground">
          Alle ingredienser ser unike ut.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-muted-foreground">
        {groups.length} gruppe{groups.length !== 1 ? "r" : ""} med mulige duplikater.
        Velg ingrediensen du vil beholde, og klikk «Slå sammen».
      </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        {item.usageCount} oppskrift{item.usageCount !== 1 ? "er" : ""}
                      </span>
                    </button>
                  );
                })}
